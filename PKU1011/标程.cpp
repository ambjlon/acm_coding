...
bool try_it( 
    int length, /* already used length of parts */ //length[counts-1]
    int count, /* count of already used parts */ //1
...
{ 
    ...
    { 
        ...
        memset(counts, 0, sizeof(counts)); //array counts 置0
        ...
        { 
            ...
            total += (counts[type_count] = counts[i]) * (lengths[type_count] = i+1); //counts[i]*(i+1)
            type_count++; 
        } 
        ...
        { 
            ...
            for(j = 0; j < failed_count; j++) 
                if(i % failed[j] == 0) 
                    break; //假如拼不出两个原棒，那么一定拼不出4,6,8...个。若拼出4个，两两组合
				           //便拼出了2个!
